...
class UserDataViewTest(TestCase):
    '''Agenda: test that the view returns the user data for authenticated users correctly'''
    def setUp(self):
        # set up a testing client
        self.client = APIClient()
        # create users to test with
    ...
    def test_user_data(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {token}')
        # call the request with the JWT token
        response = self.client.get(reverse('user_data'))
        # ensure the response is successful
        self.assertEqual(response.status_code, 200)
    
    def test_unauthorised_user(self):
        # test the view without an authorization header
        response = self.client.get(reverse('user_data'))
        # ensure the response is unsuccessful
        self.assertEqual(response.status_code, 401)
